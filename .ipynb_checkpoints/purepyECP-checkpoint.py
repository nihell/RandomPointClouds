...
def compute_ECP_single_vertex(considered_graph,
                                   vertex_filtrations,
                                   dbg=False):
    ...
    ECC = dict()
    ECC[(0, vertex_filtrations[0])] = 1
    
    if dbg: print('\n vertex filtrations : {}'.format(vertex_filtrations))
    ...
    for edge in considered_graph[0]:
        ...
        this_filtration = (edge[1], 
                           max(vertex_filtrations[edge[0]], vertex_filtrations[0])
                          )
        filtration_of_those_simplices.append(this_filtration)
        
        # changes to ECC due to those edges:
        if dbg: print('######Change of ECC at the level : {} by: -1'.format(this_filtration))
        ECC[this_filtration] = ECC.get(this_filtration, 0) - 1
    ...
    while ( len(simplices_in_current_dimension) > 0 ):
        ...
        for i in range(len(simplices_in_current_dimension)):
            ...
            for j in range(len(common_neighs[i])):                
                ...
                # need to make it mutable for a bit
                filtration_of_this_simplex = list(filtration_of_those_simplices[i])
                for k in range(len(simplices_in_current_dimension[i])):
                    ...
                    # VR
                    if ( length_of_this_edge > filtration_of_this_simplex[0] ):
                        filtration_of_this_simplex[0] = length_of_this_edge
                    # custom vertex filtration
                    the_new_vertex_id = common_neighs[i][j]
                    if ( vertex_filtrations[the_new_vertex_id] > filtration_of_this_simplex[1] ):
                        filtration_of_this_simplex[1] = vertex_filtrations[the_new_vertex_id]
                
                # make filtration inmutable again
...
def compute_local_contributions(point_cloud, epsilon, vertex_filtrations, dbg=False):
    ...
    for i in range(len(point_cloud)):
        ...
        # enumeration needs to start from i+1 because the center is at position i
        id_neigs_of_center_vectex = [i]+[j for j,point in enumerate(point_cloud[i+1:], i+1) 
                                     if np.linalg.norm(point_cloud[i] - point) <= epsilon]

        local_ECP, number_of_simplices = compute_ECP_single_vertex(graph_i,
                                                                   vertex_filtrations[id_neigs_of_center_vectex],
                                                                   dbg=dbg)
    ...
    for single_ECP in ECP_list:
        for key in single_ECP:
            total_ECP[key] = total_ECP.get(key, 0) + single_ECP[key]
    ...
    return sorted(list(total_ECP.items()), key = lambda x: x[0]), total_number_of_simplices


def compute_local_contributions_from_graph(graph_list, vertex_filtrations_list):
    # for each point, create its local graph and find all the
    # simplices in its star
    ...
    for i, current_g in enumerate(graph_list):

        local_ECP, number_of_simplices = compute_ECP_single_vertex(current_g,
                                                                   vertex_filtrations_list[i],
                                                                   dbg=True)
...
def gaussian_density(points, sigma):
    distance_matrix = scipy.spatial.distance_matrix(points, points)
